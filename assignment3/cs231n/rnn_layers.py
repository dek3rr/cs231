...
def rnn_step_backward(dnext_h, cache):
    ...
    db = np.sum(dnexth_tanh, axis=0).T.ravel()
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        h[:,t,:], f_cache[t] = rnn_step_forward(x[:,t,:], h_prev, Wx, Wh, b)
    cache = (h0, f_cache, D)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dx_step, dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dh[:,t,:]+dprev_h, f_cache[t])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
        dx[:, t, :] = dx_step
