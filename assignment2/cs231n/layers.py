...
def softmax_loss(x, y):
    ...
    max_scores = np.max(x, axis=1, keepdims=True) #predicted scores
    x -= max_scores #avoid large exp
    
    scores_exp = np.exp(x)
    loss = max_scores.sum() - correct_class_scores.sum() + np.log(np.sum(scores_exp, axis=1)).sum()
    loss /= num_train
    
    dx = scores_exp/np.sum(scores_exp, axis=1, keepdims=True)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        running_var += (1 - momentum) * x_var
        running_mean *= momentum 
        running_mean += (1 - momentum) * x_mean
        
        x_mu = (x - x_mean)
        inv_var = 1./(x_var + eps)**0.5
...
def batchnorm_backward(dout, cache):
    ...
    dxhatdmu = -inv_var
    dvardmu = (1./N)*np.sum(-2.*x_mu, axis=0)
    
    dvar = -0.5*(inv_var**3)*np.sum(dxhat * x_mu, axis=0)


    dmu = np.sum(dxhat*dxhatdmu, axis=0) + dvar*dvardmu
    dxhatdx = inv_var
    dmudx = 1./N
    
    dvardx = 2.*x_mu/N
    dx = dxhat*dxhatdx + dmu*dmudx + dvar*dvardx
...
def layernorm_backward(dout, cache):
    ...
    x_mu, inv_var, x_hat, gamma = cache
    dxhat = dout*gamma
    dx = (1./D) * inv_var * (D*dxhat - np.sum(dxhat, axis=1,keepdims=True) - x_hat*np.sum(dxhat*x_hat, axis=1,keepdims=True))
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout*x_hat, axis=0)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        mask = np.random.choice(2, x.shape, p=[1-p, p])
        out = mask*x
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    outH = int(1 + (H + 2 * pad - FH) / stride)
    outW = int(1 + (W + 2 * pad - FW) / stride)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (inv_var, x_hat, gamma, size)
    out = gamma*x_hat + beta
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = dout.shape
    inv_var, x_hat, gamma, size = cache
    ...
    dxhat = (dout*gamma).reshape(size).T
    dx = (1./D) * inv_var * (D*dxhat - np.sum(dxhat, axis=0) - x_hat*np.sum(dxhat*x_hat, axis=0))
